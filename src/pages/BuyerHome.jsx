...
export default function BuyerHome() {
  ...
  // simple search on 'keywords' array
  const doSearch = async (e) => {
    e.preventDefault();
    if (!keyword.trim()) return;
    setSearching(true);
    try {
      const q = query(collection(db, "meals"), where("keywords", "array-contains", keyword.toLowerCase()), orderBy("createdAt", "desc"), limit(50));
      const snap = await getDocs(q);
      setMeals(snap.docs.map(d => ({ id: d.id, ...d.data() })));
      setLastDoc(null);
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
          ...
          <form onSubmit={doSearch} className="flex gap-2 w-full sm:w-auto">
            <input value={keyword} onChange={(e)=>setKeyword(e.target.value)} placeholder="Search (e.g. jollof, chicken)" className="border rounded px-3 py-2 w-64" />
            <button className="bg-white border px-3 py-2 rounded" disabled={searching}>{searching ? "Searching..." : "Search"}</button>
          </form>
        </header>
        ...
        {loading ? <div className="text-center p-6">Loading...</div> : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {meals.map(m => (
                <motion.article key={m.id} initial={{opacity:0, y:8}} animate={{opacity:1,y:0}} className="bg-white rounded-lg shadow overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <button className="bg-orange-600 text-white px-3 py-1 rounded" onClick={()=>window.location.href=`/checkout?mealId=${m.id}`}>Order</button>
                    </div>
                    {m.location && <p className="text-xs text-gray-500 mt-2">From: {m.location}</p>}
                  </div>
                </motion.article>
              ))}
            </div>
            ...
            <div className="text-center mt-6">
              {lastDoc ? <button onClick={loadMore} className="px-4 py-2 bg-white border rounded">Load more</button> : <div className="text-sm text-gray-500">No more meals</div>}
            </div>
          </>
        )}
      </div>
